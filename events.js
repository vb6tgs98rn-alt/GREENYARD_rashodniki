...
import { render, renderPurchaseRequestDraft, setStatus } from './render.js';
import { openModal, closeModal, openDrawer, closeDrawer } from './ui.js';
import {
  addApartment,
  switchApartment,
  deleteApartment,
  renameCurrentApartment,
  addCustomItem,
  deleteItem,
  updateItemField,
  applyWriteoff,
...
function bindGlobalUiEvents() {
  ...
  document.getElementById('writeoffModalClose')?.addEventListener('click', () => {
    closeModal('writeoffModal');
  });

  document.getElementById('writeoffModalCancel')?.addEventListener('click', () => {
    closeModal('writeoffModal');
  });
  ...
  document.getElementById('confirmDeleteApartment')?.addEventListener('click', async () => {
    ...
    if (pendingRemovalTarget.targetType === 'item') {
      deleteItem(pendingRemovalTarget.targetId);
    }

    if (pendingRemovalTarget.targetType === 'apartment') {
      deleteApartment(pendingRemovalTarget.targetId);
    }
...
function bindApartmentEvents() {
  ...
  document.getElementById('addCustomItem')?.addEventListener('click', async () => {
    ...
    if (!ok) {
      setStatus('Заполни название расходника.');
      return;
    }
  ...
  document.addEventListener('input', async e => {
    ...
    updateItemField(
      itemFieldInput.dataset.id,
      itemFieldInput.dataset.field,
...
function bindPurchaseEvents() {
  document.getElementById('openPurchaseRequestsModal')?.addEventListener('click', () => {
    openModal('purchaseRequestsModal');
  });

  document.getElementById('closePurchaseRequestsModal')?.addEventListener('click', () => {
    closeModal('purchaseRequestsModal');
  });
  ...
  document.getElementById('closeNewPurchaseRequestModal')?.addEventListener('click', () => {
    closeModal('newPurchaseRequestModal');
  });

  document.getElementById('cancelNewPurchaseRequest')?.addEventListener('click', () => {
    closeModal('newPurchaseRequestModal');
  });
  ...
  document.getElementById('saveNewPurchaseRequest')?.addEventListener('click', async () => {
    ...
    if (!ok) {
      setStatus('Укажи количество хотя бы для одной позиции.');
      return;
    }
...
function bindKnowledgeBaseEvents() {
  document.getElementById('openKnowledgeBase')?.addEventListener('click', () => {
    openModal('kbModal');
  });
  ...
  document.addEventListener('click', e => {
    ...
    document.querySelectorAll('#kbNav .history-chip').forEach(button => {
      button.classList.toggle('active', button === knowledgeBaseTabButton);
    });

    document.querySelectorAll('.kb-section').forEach(section => {
      section.classList.toggle('active', section.id === knowledgeBaseTabButton.dataset.kbTarget);
    });
  });
}
